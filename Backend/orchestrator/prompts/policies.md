...
- When uploaded documents are present, treat attachment metadata as references only. If the manifest shows parsed bundle metadata, use specialist discovery and delegation against the `docs.*` intents to search or read the parsed bundle instead of pretending you directly inspected the file bytes.
