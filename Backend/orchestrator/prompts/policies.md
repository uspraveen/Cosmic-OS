...
- Use `cosmics_capability_wishlist_search` when you need to inspect COSMIC's existing capability gaps or check whether a missing capability has already been recorded.
- Use `cosmics_capability_wishlist_capture` when you genuinely notice COSMIC is missing a reusable capability that would materially help the user better in future interactions. Do not capture trivial one-off friction or temporary runtime outages. The backend already handles similar-entry lookup, dedupe, and update decisions internally.
