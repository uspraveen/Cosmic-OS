"""Google Sheets Agent configuration."""
...
class GoogleSheetsAgentConfig:
    ...
    max_read_cells: int = 5000
    max_write_cells: int = 20000

    @classmethod
    def from_env(cls) -> "GoogleSheetsAgentConfig":
        return cls(
            ...
            internal_llm_api_key=(
                os.getenv("GOOGLE_SHEETS_AGENT_INTERNAL_LLM_API_KEY")
                or os.getenv("GOOGLE_DOCS_AGENT_INTERNAL_LLM_API_KEY")
            ...
            internal_llm_base_url=normalize_openai_compatible_base_url(
                (
                    os.getenv("GOOGLE_SHEETS_AGENT_INTERNAL_LLM_BASE_URL")
                    or os.getenv("GOOGLE_DOCS_AGENT_INTERNAL_LLM_BASE_URL")
            ...
            enable_internal_llm=_env_bool("GOOGLE_SHEETS_AGENT_ENABLE_INTERNAL_LLM", True),
            request_timeout_sec=max(
                5.0,
                _env_float("GOOGLE_SHEETS_AGENT_REQUEST_TIMEOUT_SEC", 30.0),
            ),
            max_search_results=max(
                1,
                min(50, _env_int("GOOGLE_SHEETS_AGENT_MAX_SEARCH_RESULTS", 10)),
            ),
            max_read_cells=max(
                100,
                _env_int("GOOGLE_SHEETS_AGENT_MAX_READ_CELLS", 5000),
            ),
            max_write_cells=max(
                100,
                _env_int("GOOGLE_SHEETS_AGENT_MAX_WRITE_CELLS", 20000),
            ),
        )
