...
async def main() -> None:
    config = ImageGeneratorAgentConfig.from_env()
    if not config.redis_url:
        raise RuntimeError("REDIS_URL is not configured for image_generator_agent.")

    redis_client = create_redis_client(config.redis_url)
    agent = ImageGeneratorAgent(redis_client=redis_client, config=config)
    try:
