...
async def main() -> None:
    config = TabularAgentConfig.from_env()
    if not config.redis_url:
        raise RuntimeError("REDIS_URL is not configured for tabular_agent.")

    redis_client = create_redis_client(config.redis_url)
    agent = TabularAgent(redis_client=redis_client, config=config)
    try:
