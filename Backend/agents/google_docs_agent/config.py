...
class GoogleDocsAgentConfig:
    ...
    def from_env(cls) -> "GoogleDocsAgentConfig":
        return cls(
            ...
            internal_llm_model=(os.getenv("GOOGLE_DOCS_AGENT_INTERNAL_LLM_MODEL") or "gpt-5.6-luna").strip()
            or "gpt-5.6-luna",
            internal_llm_reasoning_effort=(
                os.getenv("GOOGLE_DOCS_AGENT_INTERNAL_LLM_REASONING_EFFORT") or "xhigh"
            ).strip()
            or "xhigh",
